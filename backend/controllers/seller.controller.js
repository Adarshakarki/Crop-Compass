...
const SELLER_OTP_EXP_MINUTES = 10;
const sellerOtpStore = new Map();

const generateSellerOtp = () =>
...
export const sendSellerOtp = async (req, res) => {
  try {
    ...
    const expiresAt = Date.now() + SELLER_OTP_EXP_MINUTES * 60 * 1000;

    sellerOtpStore.set(email, { otp, expiresAt, verified: false });

    const mailResult = await sendMail({
      to: email,
      subject: "Your seller email verification OTP",
      text: `Your verification code is ${otp}. It will expire in ${SELLER_OTP_EXP_MINUTES} minutes.`,
    });

    if (mailResult?.skipped) {
      console.error("Seller OTP email not sent:", mailResult);
      return res
        .status(500)
...
export const verifySellerOtp = async (req, res) => {
  try {
    ...
    const record = sellerOtpStore.get(email);
    ...
    sellerOtpStore.set(email, {
      verified: true,
      expiresAt: Date.now() + SELLER_OTP_EXP_MINUTES * 60 * 1000,
    });
    ...
    console.error("Error verifying seller OTP:", error);
...
export const handlesellerRegister = async (req, res) => {
  try {
    ...
    const otpRecord = sellerOtpStore.get(email);
    if (
      !otpRecord ||
      !otpRecord.verified ||
      ...
      return res.status(400).json({
        error: "Please verify your email with OTP before registration",
      });
    }
    ...
    res
      ...
      .status(201)
      .json({ success: true, message: "User created successfully", role: newSeller.role });
...
export const handlesellerlogin = async (req, res) => {
  try {
    ...
    res
      ...
      .status(200)
      .json({ success: true, message: "Login successful", role: seller.role });
  } catch (error) {
...
export const sellergetprofile = async (req, res) => {
  try {
    const sellerId = req.user._id; // Assuming authentication middleware attaches user ID to req.user
...
export const getSellerItems = async (req, res) => {
  try {
    ...
    const items = await Product.find(filter); // Filter products by seller and category
